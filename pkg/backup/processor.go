...
func (p *LogicalBackupProcessor) IsValidBackupFile(fileName string, logger logr.Logger) bool {
	if !strings.HasPrefix(fileName, "backup.") || (!strings.HasSuffix(fileName, ".sql") && !strings.HasSuffix(fileName, ".sql.gz")) {
		logger.Info("File has no backup. prefix or .sql suffix or .sql.gz", "file", fileName)
		return false
	}
